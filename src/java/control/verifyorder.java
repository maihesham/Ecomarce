           ...
           int id=Integer.parseInt(x);
            ...
            ArrayList<getorderall> verify=new ArrayList<getorderall>();
              verify=(ArrayList<getorderall> ) session.getAttribute("ver");
               ArrayList<getorderall> newverify=new ArrayList<getorderall>();
               session.removeAttribute("ver");
               for(int i=0;i<verify.size();i++){
                   if(verify.get(i).get_orderid()!=id){
                       newverify.add(verify.get(i));
                   }
               }
               session.setAttribute("ver", newverify);
        } catch (ClassNotFoundException ex) {
            Logger.getLogger(verifyorder.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
